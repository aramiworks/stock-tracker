...
describe("useTrackerCatalogBrowseStore", () => {
  ...
  it("starts with an empty set of selected unit ids", () => {
    expect(useTrackerCatalogBrowseStore.getState().selectedUnitIds.size).toBe(0);
  });

  it("toggleUnit adds an id when not present", () => {
    useTrackerCatalogBrowseStore.getState().toggleUnit("unit-1");
    expect(useTrackerCatalogBrowseStore.getState().selectedUnitIds.has("unit-1")).toBe(true);
  });

  it("toggleUnit removes an id when already present", () => {
    useTrackerCatalogBrowseStore.getState().toggleUnit("unit-1");
    useTrackerCatalogBrowseStore.getState().toggleUnit("unit-1");
    expect(useTrackerCatalogBrowseStore.getState().selectedUnitIds.has("unit-1")).toBe(false);
  });
