...
describe("purchaseCreateInputSchema", () => {
  ...
  it("rejects a non-positive amount", () => {
    expect(
      purchaseCreateInputSchema.safeParse({ ...validCreate, amount: 0 })
        .success,
    ).toBe(false);
  });
...
describe("purchaseUpdateInputSchema", () => {
  ...
  it("rejects an empty itemName", () => {
    expect(purchaseUpdateInputSchema.safeParse({ itemName: "" }).success).toBe(
      false,
    );
  });
...
describe("purchaseOutputSchema", () => {
  ...
  it("rejects a non-uuid id", () => {
    expect(
      purchaseOutputSchema.safeParse({ ...validOutput, id: "nope" }).success,
    ).toBe(false);
  });
