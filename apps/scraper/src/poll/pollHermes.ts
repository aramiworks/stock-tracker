...
async function emitDropEvent(
  ...
  if (!ingest) {
    ...
    console.warn(
      `[pollHermes] transition on sku ${skuId} but no ingest client configured — drop event not emitted`,
    );
    return false;
  }
  try {
    ...
    console.warn(
      `[pollHermes] drop-event ingest failed for sku ${skuId} (pending INF-1356): ${message}`,
    );
    return false;
  }
}
...
/**
 * Poll a single Hermès SKU: build URL, fetch, parse, record state, and emit a
 ...
 */
export async function pollHermesSku(
  deps: PollSkuDeps,
...
/**
 * Poll every active Hermès SKU in the catalog. Drives pollHermesSku over the
 * SKUs returned from Prisma (active, with a reference code, under an active
 * Hermès watchable unit).
 */
export async function pollHermes(
  deps: PollHermesDeps,
): Promise<PollSkuResult[]> {
  ...
  for (const sku of skus) {
    results.push(
      await pollHermesSku(deps, adapter, {
        id: sku.id,
