...
/**
 * Starts an auth-service tRPC server on a random port for e2e testing.
 ...
 */
export async function startAuthTestServer(): Promise<{
  url: string;
  close: () => Promise<void>;
}> {
  ...
  // Use standalone tRPC HTTP server (same pattern as tracker-service tests)
